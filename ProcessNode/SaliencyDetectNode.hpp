...
SaliencyDetectNode<ImageSigT>::SaliencyDetectNode()
{
	...
	setOutputPort(makeOutputSignal(),OUTPUT_PORT_SALIENCY_IMAGE_DATA);

	//////////////////////////////////////////////////////////////////////////
	//build monitor var
	EAGLEEYE_MONITOR_VAR_EXT(int,setSaliencyType,getSaliencyType,"saliency_type","min=0 max=1");
	EAGLEEYE_MONITOR_VAR(int,setQuantizedNum,getQuantizedNum,"quantize_num");
	//////////////////////////////////////////////////////////////////////////
}
...
template<class ImageSigT>
Matrix<float> SaliencyDetectNode<ImageSigT>::getSaliencyMap()
{
	ImageSignal<float>* output_img = getOutputImageSignal();
	if (output_img)
	{
...
template<class ImageSigT>
void SaliencyDetectNode<ImageSigT>::setSaliencyType(SalienceType t)
{
	m_saliency_type = t;
}
template<class ImageSigT>
void SaliencyDetectNode<ImageSigT>::setSaliencyType(const int index)
{
	if (index < 0 || index > 1)
	{
		return;
	}
	m_saliency_type = SalienceType(index);
}
template<class ImageSigT>
void SaliencyDetectNode<ImageSigT>::getSaliencyType(int& index)
{
	index = int(m_saliency_type);
}

template<class ImageSigT>
void SaliencyDetectNode<ImageSigT>::executeNodeInfo()
{
	//get input image signal
	ImageSigT* input_img_sig = getInputImageSignal(INPUT_PORT_IMAGE_DATA);
	Matrix<PixelType> img_mat = input_img_sig->img;
	...
	PixelType min_val;
	PixelType max_val;
	getMaxMin(img_mat,max_val,min_val);

	//transform to matrix with float type
	Matrix<float> valid_img_mat = img_mat.transform(NormalizeOperation<PixelType,float>(min_val,max_val,0.0f,1.0f));
	m_img = cv::Mat(rows,cols,CV_32F);
	memcpy(m_img.data,valid_img_mat.dataptr(),sizeof(float) * rows * cols);
	...
	Matrix<float> saliency_img(rows,cols,(float*)m_img_saliency.data,false);
	ImageSignal<float>* output_img_sig = getOutputImageSignal(OUTPUT_PORT_SALIENCY_IMAGE_DATA);
	output_img_sig->img = saliency_img;
}

template<class ImageSigT>
bool SaliencyDetectNode<ImageSigT>::selfcheck()
{
	Superclass::selfcheck();

	if (m_saliency_type != HC && m_saliency_type != RC)
	{
		EAGLEEYE_ERROR("sorry, you set a wrong saliency type \n");
		return false;
	}

	if (AtomicTypeTrait<PixelType>::size != 1)
	{
		EAGLEEYE_ERROR("sorry,I couldn't support multi-channels data \n");
		return false;
	}
...
template<class ImageSigT>
void SaliencyDetectNode<ImageSigT>::quantize()
{
	//quantize m_img
...
template<class ImageSigT>
void SaliencyDetectNode<ImageSigT>::computeHC()
{
	quantize();
	cv::Mat weight;
...
template<class ImageSigT>
void SaliencyDetectNode<ImageSigT>::smoothSaliency(const cv::Mat& bin_color,cv::Mat& scal,float delta,const std::vector<std::vector<CostIdx>> &similarity)
{
...
template<class ImageSigT>
void SaliencyDetectNode<ImageSigT>::computeRC()
{
	//////////////////////////////////////////////////////////////////////////
	//quantization 
	quantize();
	...
	ImageSignal<unsigned char>* img_sig = new ImageSignal<unsigned char>;
	img_sig->img = used_img;
	SRMNode<ImageSignal<unsigned char>>* srm_node = new SRMNode<ImageSignal<unsigned char>>;
	srm_node->setInputPort(img_sig);
	srm_node->start();

	Matrix<int> label_map = srm_node->getLabelMap();
	int reg_num = srm_node->getLabelNum();
...
template<class ImageSigT>
void SaliencyDetectNode<ImageSigT>::regionContrast(const std::vector<_Region> &regs,
...
template<class ImageSigT>
void SaliencyDetectNode<ImageSigT>::buildRegion(const cv::Mat& reg_label,
...
template<class ImageSigT>
void SaliencyDetectNode<ImageSigT>::setQuantizedNum(const int num)
{
	m_quantized_num = num;
}
template<class ImageSigT>
void SaliencyDetectNode<ImageSigT>::getQuantizedNum(int& num)
{
	num = m_quantized_num;
}
}