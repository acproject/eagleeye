...
static const float atan2_p1 = 0.9997878412794807f*(float)(180/EAGLEEYE_PI);
static const float atan2_p3 = -0.3258083974640975f*(float)(180/EAGLEEYE_PI);
static const float atan2_p5 = 0.1555786518463281f*(float)(180/EAGLEEYE_PI);
static const float atan2_p7 = -0.04432655554792128f*(float)(180/EAGLEEYE_PI);
...
{
    int i = 0;
    float scale = angleInDegrees ? 1 : (float)(EAGLEEYE_PI/180);

#ifdef __SSE__
	Eagleeye32suf iabsmask; iabsmask.i = 0x7fffffff;
                        ...
                        int len, int angle_in_degrees )
{
    ...
    static const double k2 = (2*EAGLEEYE_PI)/N;
...
#define EXPPOLY(x)  \
    ...
    int i = 0;
    const Eagleeye32suf* x = (const Eagleeye32suf*)_x;
    Eagleeye32suf buf[4];
    ...
    if( n >= 8)
    {
        ...
        eagleeye_ushort EAGLEEYE_DECL_ALIGNED(16) tab_idx[8];
...
#define EXPPOLY(x)  (((((A0*(x) + A1)*(x) + A2)*(x) + A3)*(x) + A4)*(x) + A5)
    ...
    int i = 0;
    Eagleeye64suf buf[4];
    const Eagleeye64suf* x = (const Eagleeye64suf*)_x;
    ...
    if( USE_SSE2 )
    {
        ...
        int EAGLEEYE_DECL_ALIGNED(16) tab_idx[4];
    ...
    for( ; i <= n - 4; i += 4 )
    {
        ...
        buf[0].i = (eagleeye_int64)t << 52;

        t = (val1 >> EXPTAB_SCALE) + 1023;
        t = !(t & ~2047) ? t : t < 0 ? 0 : 2047;
        buf[1].i = (eagleeye_int64)t << 52;

        t = (val2 >> EXPTAB_SCALE) + 1023;
        t = !(t & ~2047) ? t : t < 0 ? 0 : 2047;
        buf[2].i = (eagleeye_int64)t << 52;

        t = (val3 >> EXPTAB_SCALE) + 1023;
        t = !(t & ~2047) ? t : t < 0 ? 0 : 2047;
        buf[3].i = (eagleeye_int64)t << 52;
...
static const double EAGLEEYE_DECL_ALIGNED(16) icvLogTab[] = {
...
static void Log_32f( const float *_x, float *y, int n )
{
    ...
    if( USE_SSE2 )
    {
        ...
        int EAGLEEYE_DECL_ALIGNED(16) idx[4];
...
static void iPow8u(const eagleeye_uchar* src, eagleeye_uchar* dst, int len, int power)
{
    iPow_<eagleeye_uchar, int>(src, dst, len, power);
}

static void iPow8s(const eagleeye_schar* src, eagleeye_schar* dst, int len, int power)
{
    iPow_<eagleeye_schar, int>(src, dst, len, power);
}

static void iPow16u(const eagleeye_ushort* src, eagleeye_ushort* dst, int len, int power)
{
    iPow_<eagleeye_ushort, int>(src, dst, len, power);
}
...
typedef void (*IPowFunc)( const eagleeye_uchar* src, eagleeye_uchar* dst, int len, int power );
